...
import org.mybatis.generator.dto.Constants;
...
//            ConfigurationParam config = cp.parseConfiguration(configurationFile);
//
...
//    }
    ...
    public void run(ConfigurationParam configuration) {
        ...
        //String configfile = arguments.get(CONFIG_FILE);
//        File configurationFile = new File("C:\\临时存储\\新建文件夹\\mybatis-generator-core\\src\\main\\resources\\generatorConfig.xml");
        ...
        Set<String> contexts = new HashSet<String>();
        contexts.add("DB2Tables");


        try {
//            ConfigurationParser cp = new ConfigurationParser(warnings);
//            ConfigurationParam config = cp.parseConfiguration(configurationFile);
            Constants.author = configuration.getAuthor();
            Configuration testConfig = new Configuration();
            Context context = new Context(null);
            JDBCConnectionConfiguration jdbcConnectionConfiguration = new JDBCConnectionConfiguration();
            jdbcConnectionConfiguration.setConnectionURL(configuration.getConnectionUrl());
            jdbcConnectionConfiguration.setDriverClass(configuration.getDriveClass());
            jdbcConnectionConfiguration.setPassword(configuration.getDatabasePassword());
            jdbcConnectionConfiguration.setUserId(configuration.getDatabaseUserId());

            JavaModelGeneratorConfiguration javaModelGeneratorConfiguration = new JavaModelGeneratorConfiguration();
            javaModelGeneratorConfiguration.setTargetPackage(configuration.getJavaFileTatgetPackage());
            javaModelGeneratorConfiguration.setTargetProject(configuration.getJavaFileTatgetProject());

            JavaClientGeneratorConfiguration javaClientGeneratorConfiguration = new JavaClientGeneratorConfiguration();
            javaClientGeneratorConfiguration.setTargetPackage(configuration.getMapperFileTatgetPackage());
            javaClientGeneratorConfiguration.setTargetProject(configuration.getJavaFileTatgetProject());
            javaClientGeneratorConfiguration.setConfigurationType(configuration.getMapperFileType());
           // javaClientGeneratorConfiguration.setImplementationPackage();

            SqlMapGeneratorConfiguration sqlMapGeneratorConfiguration = new SqlMapGeneratorConfiguration();
            sqlMapGeneratorConfiguration.setTargetPackage(configuration.getXmlFileTatgetPackage());
            sqlMapGeneratorConfiguration.setTargetProject(configuration.getXmlFileTatgetProject());
            ...
            tableConfiguration.setTableName(configuration.getTableName());
            tableConfiguration.setDomainObjectName(configuration.getPoName());
            context.addTableConfiguration(tableConfiguration);
             testConfig.addContext(context);

            testConfig.addClasspathEntry("C:\\kyle\\projects\\aiways-tsp\\hu\\hu-service\\src\\main\\resources\\mybatis\\mysql-connector-java-5.1.44.jar");

            DefaultShellCallback shellCallback = new DefaultShellCallback(
                    true);

            MyBatisGenerator myBatisGenerator = new MyBatisGenerator(testConfig, shellCallback, warnings);

            ProgressCallback progressCallback = true ? new VerboseProgressCallback()
                    : null;
